...
void DescrTabla::activar_mi()
{
   ...
   //
   // .....
   fijarPuntero(datos); // especifica localizacion y formato
...
void DescrTabla::activar_md()
{
   ...
   if (nombre_vbo == 0){ // 1.
      ...
      glBindBuffer (tipo_tabla, nombre_vbo); // 2.
   
   if (tipo_tabla == GL_ARRAY_BUFFER){ // 2.2
...
void ArrayVertices::visualizarGL_MI_BVE( const GLenum tipo_primitiva ) 
{
   ...
   for (GLuint i = 0; i < nv; i++){ // recorre todos los vertices
      // recuperar indice de vertice en la tabla de coordenadas (iv)
      const GLuint iv = (indices->datos != nullptr)? *(((const GLuint *) indices->datos) + i) : i;
      
      //enviar atributos de vertices que corresponda
      if (colores != nullptr)
...
void ArrayVertices::visualizarGL_MI_DAE( const GLenum tipo_primitiva ) 
{
   ...
   deshabilitar_tablas(); // deshabilita todas las tablas
   ...
   // invocar glDrawElements o glDrawArrays en funcion de si hay o no hay indices
   if (indices != nullptr)
      glDrawElements (tipo_primitiva, num_indices, indices->tipo_valores, indices->datos);
   else
      glDrawArrays(tipo_primitiva, 0, num_vertices);

   // dejar tablas deshabilitadas para visualizar otros arrays de vertices
   deshabilitar_tablas();
...
void ArrayVertices::visualizarGL_MD_VAO( const GLenum tipo_primitiva ) 
{
   ...
   if (nombre_vao == 0){ // si el VAO no estaba creado
      ...
      coordenadas->activar_md(); // activar las tablas que proceda (incluyendo indices)
